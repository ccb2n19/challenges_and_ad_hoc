...
station$string <- "Morden Underground Station"
...
osm$places$subset <- osm$places$response %>%
  ...
  st_filter(
    station$map_area %>% st_buffer(-500)
  )

osm$places$remove <- osm$places$subset %>%
  st_filter(
    station$immediate_area
  )

osm$places$subset <- osm$places$subset %>%
  filter(
    !name %in% osm$places$remove$name
  )
...
scale$line <- tibble(
  lon = c(station$map_area %>% st_bbox() %>% extract(1), station$map_area %>% st_bbox() %>% extract(1) + 5000),
  lat = c(station$map_area %>% st_bbox() %>% extract(2), station$map_area %>% st_bbox() %>% extract(2))
) %>%
  st_as_sf(
    coords = c("lon", "lat"),
    crs = 27700
...
plot <- ggplot() +
  ...
  geom_sf_label(
    data = osm$rail$subset %>% filter(name %>% str_detect(station$string %>% word()), !is.na(line)),
    aes(
      label = name
    ),
    nudge_y = 350,
...
ggsave(
  filename = path %>% paste0("/plot/", station$string %>% word(1), "_public_transport_accessibility.png"),
  plot = plot,
  height = dim$h * 4,
  width = dim$w * 4,
  units = "px"
